 ...
 * holds-service: the API Gateway is the sole point that authenticates and authorizes
 * incoming traffic (see {@code api-gateway}'s {@code SecurityConfig}); holds-service only
 ...
 * "authenticated by default" filter chain so holds-service's own HTTP layer keeps behaving
